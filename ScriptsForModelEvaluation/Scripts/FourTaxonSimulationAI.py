...
if __name__ == '__main__':
    ...
    for t_selection in SelectionTime:
        ...
        for str_m in Migration:
            for i in range(1, RepNum):
                file_name = str_t12 + '_' + str_t123 + '_' + str_t_selection + '_' + str_m + '_' + '32' + '_' + str(i)
                ...
                if str_m == '0':
                    ...
                    if SelectionCoefficient == 0.0:
                        print(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234,
                            Rec_com, file_name))
                    ...
                    # topo with selective sweep in P2
                        ...
                        print(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 %s -Sp 0.5 -SI %s 4 0 0 0 0 -Smu %s -Sc 0 2 %s %s 0 -N %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, 
                    ...
                    if SelectionCoefficient == 0.0:
                        print(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 -m 2 3 %s %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, str_m,
                            Rec_com, file_name))
                    ...
                    # topo with adaptive introgression from P3 to P2
                        ...
                        print(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 -m 2 3 %s %s -SAA %s -SAa %s -Sp 0.5 -SI %s 4 0 0 1 0 -N %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, str_m,
    ...
    # part 2 topo (((1,2),3),4) introgression from 2 to 3
    for t_selection in SelectionTime:
        str_t_selection = str(float(t_selection) * float(str_t12))
        for str_m in Migration:
            for i in range(1, RepNum):
                file_name = str_t12 + '_' + str_t123 + '_' + str_t_selection + '_' + str_m + '_' + '23' + '_' + str(i)
                OutputFile.write('seq_' + file_name + '\n')
                TreeOutputFile.write('tree_' + file_name + '\n')
                #labels.write(str_m + '	0.0	' + str((10 - m) / 10.0) + '\n')
                if str_m == '0':
                    if SelectionCoefficient == 0.0:   
                        ...
                        os.system(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 %s -Sp 0.5 -SI %s 4 0 0 0 0 -Smu %s -Sc 0 3 %s %s 0 -N %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                        PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, 
                        Rec_com, str_t_selection, SubRate, SelectionCoefficient, SelectionCoefficient, PopSize, file_name))
                else:
                    if SelectionCoefficient == 0.0:
                        os.system(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 -m 3 2 %s %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, str_m,
                            Rec_com, file_name))
                    else:
                        os.system(
                        "java -jar %s -ms %s -ej %s 2 1 -ej %s 3 1 -ej %s 4 1 -m 3 2 %s %s -SAA %s -SAa %s -Sp 0.5 -SI %s 4 0 1 0 0 -N %s -T | tail -n +4 | grep -v // | head -n -2 > tree_%s" % (
                            PATHTOMSMS, Indv_com, str_t12, str_t123, str_t1234, str_m,
                            Rec_com, SelectionCoefficient, SelectionCoefficient, str_t_selection, PopSize, file_name))
                partitions = commands.getoutput('wc -l tree_' + file_name)
                os.system("%s -q -mHKY %s -p %s < tree_%s > seq_%s" % (
